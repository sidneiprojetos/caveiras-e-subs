...
const EventForm: React.FC<{ event: AgendaEvent | null; divisoes: Divisao[]; onClose: () => void; onSave: (event: AgendaEvent) => void }> = ({ event, divisoes, onClose, onSave }) => {
  const [form, setForm] = useState<AgendaEvent>(event || { id: `event-${Date.now()}`, title: '', date: toDateInput(new Date()), time: '', type: 'REUNIAO', location: '', divisaoId: '', description: '', createdAt: new Date().toISOString() });
