...
export function printMemberDossier(member: Member): Promise<boolean> {
  ...
  const html = `
      ...
      <div style="flex: 1;">
        ...
        <div style="margin-top: 6px;">
          ...
          <span style="font-size: 9.5pt; font-weight: 700; color: #444444; margin-left: 10px;">
            Colete: <strong style="font-family: 'Teko', sans-serif; font-size: 13pt; color: #991b1b;">${member.coleteNumber || 'Sem Colete Oficial'}</strong>
          </span>
        </div>
      </div>
    </div>
    ...
    <div class="info-grid">
      ...
      <div class="info-item">
        <span class="info-label">Número da Matrícula / Colete</span>
        <span class="info-value">${member.coleteNumber || 'Não atribuído'}</span>
      </div>
      <div class="info-item">
    ...
    <div class="section-title">2. Contato & Identificação</div>
    <div class="info-grid">
      <div class="info-item">
...
export function printRosterReport(members: Member[], filterTitle = 'Quadro Geral de Efetivo'): Promise<boolean> {
  ...
  const rowsHtml = members.map((m, idx) => `
    <tr>
      ...
      <td>${m.name}</td>
      <td style="font-weight: 700; font-family: 'Teko', sans-serif; font-size: 11pt; color: #991b1b;">${m.coleteNumber || '-'}</td>
  ...
  const cardsHtml = members.map((m) => `
    <div class="id-card">
      <div class="id-card-header">
        ...
        <div style="font-size: 13pt; font-weight: 900; font-family: 'Teko', sans-serif; color: #991b1b; border: 1px solid #991b1b; padding: 0 5px; border-radius: 4px;">
          ${m.coleteNumber || 'S/N'}
        </div>
      </div>
